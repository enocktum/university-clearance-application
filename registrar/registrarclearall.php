                ...
                <div class="row">
                    <div class="col-lg-12">
						...
						if($show)
						{
							...
							{
								 $ch=mysqli_query($con,"select * from department_registrar where idnumber='$studentid'");
								 $hc=mysqli_fetch_array($ch);
								 $basket=$hc['basket'];
								 if($basket=='')
								 {
								 	//start of bookstore clearance check
								$bookstorecheck=mysqli_query($con,"select * from department_bookstore where idnumber='$studentid' && basket='' && clearance_status='1'");
											...
											if($foodno==1)
											{
												...
												if($healthno==1)
												{
													...
													if($libraryno==1)
													{
														...
														$physicalplantcheck=mysqli_query($con,"select * from department_physicalplant where idnumber='$studentid' && basket='' && clearance_status='1'");
