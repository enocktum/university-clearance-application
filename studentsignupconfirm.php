...
<div class="wrapper col4">
  ...
  <fieldset style="text-align:center;">
	...
	{
		$query=mysqli_query($con,"select * from student_details where idnumber='$username'");
		$ngapi=mysqli_num_rows($query);
		if($ngapi==0)
		{
			$insertstudentdetails=mysqli_query($con,"insert into student_details (year,name,residence,major,idnumber,password,department,clearance_status) values ('$year','$name','$residence','$major','$username','$password','$department','2')");
			if($insertstudentdetails)
			{
				$insertbookstore=mysqli_query($con,"insert into department_bookstore (idnumber,clearance_status,basket) values ('$username','2','')");
				if($insertbookstore)
				{
					...
					if($insertdos)
					{
						...
						if($insertfinance)
						{
							$insertfood=mysqli_query($con,"insert into department_food (idnumber,clearance_status,basket) values ('$username','2','')");
							if($insertfood)
							{
								...
								if($inserthealth)
								{
									...
									if($insertlibrary)
									{
										$insertphysicalplant=mysqli_query($con,"insert into department_physicalplant (idnumber,clearance_status,basket) values ('$username','2','')");
